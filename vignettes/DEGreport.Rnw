...
\Rcode{DEGSet} is a class to store the DE results like the one from
\Rcode{results()} function. DESeq2 offers multiple way to ask for
contrasts/coefficients. With \Rcode{degComps} is easy to get multiple
...
\Rcode{degs} contains 3 elements, one for each contrast/coefficient asked for.
It contains the results output in the element \Rcode{raw} and the output of
\Rcode{lfcShrink} in the element \Rcode{shrunken}.
To obtain the results from one of them, use the method \Rcode{dge}:
...
\Rcode{degDefault}, that contains the default table to get.
